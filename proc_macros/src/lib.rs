...
fn impl_server(tr: &ItemTrait) -> Result<TokenStream, Rejections> {
    ...
    let handlers = methods.iter().map(|method| {
        ...
        let try_serialize = quote_spanned! {
            method_return_type_span =>
                easy_jsonrpc_mw::try_serialize(&result)
        };
    ...
    Ok(quote! {
        impl easy_jsonrpc_mw::Handler for dyn #trait_name {
            fn handle(&self, method: &str, params: easy_jsonrpc_mw::Params)
                      -> Result<easy_jsonrpc_mw::Value, easy_jsonrpc_mw::Error> {
                match method {
                    #(#handlers,)*
                    _ => Err(easy_jsonrpc_mw::Error::method_not_found()),
                }
            }
        }
    })
...
fn impl_client_method(method: &MethodSig) -> Result<TokenStream, Rejections> {
    ...
    let args_serialize: &Vec<_> = &args
        ...
        .map(|(i, (name, _))| {
            let arg_num_name = Ident::new(&format!("arg{}", i), name.span());
            quote! {
                easy_jsonrpc_mw::serde_json::to_value(#arg_num_name).map_err(|_| easy_jsonrpc_mw::ArgSerializeError)?
            }
        })
        .collect();
    ...
    Ok(quote! {
        ...
        pub fn #method_name ( #(#fn_definition_args,)* )
                                 -> Result<easy_jsonrpc_mw::BoundMethod<'static, #return_typ>, easy_jsonrpc_mw::ArgSerializeError> {
...
fn add_handler(trait_name: &Ident, method: &MethodSig) -> Result<TokenStream, Rejections> {
    ...
    let parse_args = args.iter().enumerate().map(|(index, (ident, ty))| {
        ...
        quote_spanned! { ty.span() => #prefix {
            ...
            easy_jsonrpc_mw::serde_json::from_value(next_arg).map_err(|_| {
                easy_jsonrpc_mw::InvalidArgs::InvalidArgStructure {
    ...
    Ok(quote! {{
        let mut args: Vec<easy_jsonrpc_mw::Value> =
